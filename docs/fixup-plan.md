...
- 连续 3 次失败后进入账号级短熔断，建议冷却 30~120 分钟；明确风险按类型分级：登录失效 15 分钟、操作频繁 90 分钟、滑块验证 2 小时、验证码/安全验证 12 小时；身份指纹漂移只记录并返回错误，不触发账号级冷却熔断
...
**数值参数说明：** 交互节奏参数（如浏览 6s/12s/25s/45s、评论前 45s 阅读等）参考了 [3][6] 的设计思路；账号级风险冷却使用代码中的分级策略，不再共用固定长冷却，后续根据 `LastRiskText` 和平台反馈继续校准。
